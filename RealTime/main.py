...
from Parameters import num_vehicles, vehicle_capacity, distance_matrix, initial_temperature, cooling_rate, max_iterations, max_recent_costs, identical_solution_threshold
import pandas as pd
import math
import random
import copy
    ...
    while possible:
        ...
        for demand in repairing_demands:
            for vehicle_id in range(num_vehicles):
                best_update, cost = vehicle_list[vehicle_id].insert_demand(demand,demandlist)
                if best_update:
                    print(cost)
                    if cost < best_cost:
                        best_cost = cost
...
def random_repair(vehicle_list: list[Vehicle], repairing_demands: list[Demand],demandlist: DemandList, optional_demands: list[Demand]):
    ...
    while failure_count < max_failure and repairing_demands:
        ...
        if new_vehicle:
            ...
            for test_vehicle in new_vehicle_list:
                for node in test_vehicle.get_route().nodes:
                    print(demandlist.get_demand_by_node(node).index," ", test_vehicle.get_index())
...
def calculate_cost(vehicle_list: list[Vehicle], demands: DemandList):
    ...
    for vehicle in vehicle_list:
        ...
        node_count += len(vehicle.get_route().nodes)
...
def print_solution(vehicle_list: list[Vehicle], demands: DemandList):
    ...
    total_cost, nodes_serviced= calculate_cost(vehicle_list, demands)
    print("total cost: ",total_cost, "nodes_serviced", nodes_serviced,"\n")
...
def ALNS(initial_demands: DemandList, new_demands: DemandList, max_iterations: int, start_node: int, end_node: int):
    ...
    temporary_best_cost, temporary_most_nodes = calculate_cost(temporary_best_solution, demands)
    print(temporary_best_cost, temporary_most_nodes)
    destroy_rate = 0.2
    temperature = initial_temperature
    best_cost = temporary_best_cost
    best_solution = temporary_best_solution
    most_nodes = temporary_most_nodes
    ...
    while iteration < max_iterations:
        ...
        if need_random_destroy:
            ...
            if success:
                ...
                print_solution(vehicle_list, demands)
                recent_costs.clear()
            random_cost, random_nodes_serviced = calculate_cost (vehicle_list, demands)
            temporary_best_cost = random_cost
            temporary_most_nodes = random_nodes_serviced
            ...
            iteration += 1
            current_cost, nodes_serviced = calculate_cost(current_solution, demands)
            cost_diff = current_cost - temporary_best_cost
            if(cost_diff < 0 and nodes_serviced == most_nodes )or (nodes_serviced > temporary_most_nodes)or random.random() < math.exp(-cost_diff - 10 * (most_nodes - nodes_serviced)/ temperature):
                vehicle_list = current_solution
                temporary_best_cost = current_cost
                temporary_most_nodes = nodes_serviced
            ...
            if (temporary_best_cost < best_cost and temporary_most_nodes == most_nodes) or most_nodes < temporary_most_nodes:
                best_cost = temporary_best_cost
...
def main():
    ...
    new_demands = read_new_demand("Data/new_demands.txt",num_initial_demands)
