...
def print_solution(vehicle_list: list[Vehicle], demands: DemandList):
    ...
    total_cost, load_serviecd= calculate_cost(vehicle_list, demands)
    print("total cost: ",total_cost, "load_serviecd", load_serviecd,"\n")
    
def ALNS(initial_demands: DemandList, new_demands: DemandList, max_iterations: int, start_node: int, end_node: int,num_vehicles: int, time_limit):
    ...
    time_limit = time_start + time_limit
    iteration = 0
    recent_costs= []
    ...
    while iteration < max_iterations and time.time() < time_limit:
        print("\nIteration: ",iteration)
        if need_random_destroy:
            ...
            if success:
                ...
                print_solution(vehicle_list, demands)
                recent_costs.clear()
            random_cost, random_load_serviecd = calculate_cost (vehicle_list, demands)
            temporary_best_cost = random_cost
            temporary_most_load = random_load_serviecd
            ...
            iteration += 1
            current_cost, load_serviecd = calculate_cost(current_solution, demands)
            cost_diff = current_cost - temporary_best_cost
            if(cost_diff < 0 and load_serviecd == most_load )or (load_serviecd > temporary_most_load)or random.random() < math.exp(-cost_diff -   (most_load - load_serviecd)/ temperature):
                vehicle_list = current_solution
                temporary_best_cost = current_cost
                temporary_most_load = load_serviecd
        ...
        for a in [3,6,9]:
            for b in [0,0.5,1]:
                ...
                with open(output_file, 'w') as f:
                    ...
                    sys.stdout = f
                    ALNS(initial_demands, new_demands, max_iterations, start_node_id, end_node_id, num_vehicles, 10* k * a )
